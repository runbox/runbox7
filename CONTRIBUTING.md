...
7. Run the full test suite (`npm run ci-tests`), and ensure that all tests pass.
   **Note:** `npm run build` auto-generates content for `src/app/changelog/changes.ts` from the git log on deploy. Do not edit this file.
...
* **feat**: A new feature
* **feature**: A new feature (alias for feat)
* **fix**: A bug fix
