...
import {SecurityContext,Component,Input,Output,EventEmitter,NgZone, ViewChild} from '@angular/core';
...
//@Input() messageActionsHandler : MessageActions;
  ...
  public is_tld_valid = function () {
    ...
    var rgx = "\\.("+this.tld_list.join("|")+")$";
    var rgx_tld = new RegExp( rgx );
    var result = this.domain_wanted.match(rgx_tld)
    return result
  }
  ...
  public check_avail = function () {
    ...
    if ( ! this.is_tld_valid() ) {
        ...
        return this.show_error("The top level domain (TLD) you selected is invalid. Please check the available ones and check again.", 'Dismiss') 
    }
    this.is_available = null
    var d = this.parse_domain_wanted()
    if ( ! d || ! d.sld || ! d.tld ) {
        return this.show_error("The domain you selected is in invalid format or has invalid characters. Example of format: xyz.com xyz.co.uk", 'Dismiss') 
    }
    var sld = d.sld
    var tld = d.tld
  ...
  public parse_domain_wanted = function () {
    var match = this.domain_wanted.match(/^([^\.]+)\.(.+)$/);
    if ( ! match ) { return {} }
    var sld = match[1];
    var tld = match[2];
  ...
  public update_specific_doc_value = function ( ev, docs ) {
    if ( docs.type == 'select' ) {
        ...
        var selected_option;
        var selected_value = docs.value
        if ( docs.field == 'RegistrantCountry' ) {
            var country = docs.value
            var zip_code_regex = this.zip_code_regex_by_country()[country]
        ...
        if ( this.required_by[docs.name] ) {
            ...
            for ( var i = 0, item; item = this.required_by[ docs.name ][ i++ ] ; ) {
                this.required_fields[ item ] = 0
            }
        }
        this.required_by[docs.name] = []
        for ( var i = 0, option; option = docs.options[ i++ ] ; ) {
            if ( option.value == selected_value ) {
                selected_option = option;
                if ( option.requires && option.requires.length ) {
                    for ( var j = 0, req_field; req_field = option.requires[ j++ ] ; ) {
  ...
  public purchase = function () {
    ...
    var self = this
    if ( this.validate() ) {
        var products_selected = [this.selected_product.pid]
  ...
  public get_specific_docs_values = function () {
    ...
    if ( this.specific_docs && this.specific_docs.length ) {
        for ( var i = 0, doc; doc = this.specific_docs[ i++ ]; ) {
  ...
  public get_docs_values = function () {
    var docs = {}
    for ( var i = 0, item; item = this.generic_docs[ i++ ] ; ) {
  ...
  public get_specific_docs_hash = function () {
    var docs = {}
    for ( var i = 0, item; item = this.specific_docs[ i++ ] ; ) {
        docs[ item.name ] = item
    }
    return docs
  }
    ...
    for ( var i = 0, item; item = this.generic_docs[ i++ ] ; ) {
        if ( ( item.type == 'text' || item.type == 'select' ) ) {
            ...
            if ( item.pattern && item.value && item.value.length ) {
                var rgx = new RegExp( item.pattern );
  ...
  public is_normal_doc_required ( doc ) {
    var docs_values = this.get_docs_values();
...
//}
  ...
  public load_domreg_hash () {
      var hash = window.location.href.match(/domreg_hash=([^&]+)/)
      if ( hash && hash[1] ) {
          this.domreg_hash = hash[1]
          let headers = new Headers();
          headers.append('Accept', 'application/json');
          let url = "/rest/v1/domain_registration/enom/domreg_hash/"+this.domreg_hash;
  ...
  public activate_renew_domain () {
      var is_renew_domain = window.location.href.match(/renew_domain=([^&]+)/)
  ...
  public update_renewal_price (type, option) {
    ...
    for ( var prod_type in this.renewal_selected_product ) {
  ...
  public load_domreg_hash_data_generic_docs() {
      for ( var k in this.domreg_data.result.data.docs ) {
          var docs_values = this.get_docs_hash();
          if ( docs_values ) {
  ...
  public load_domreg_hash_data_specific_docs() {
    var docs_hash = this.get_specific_docs_hash()
    for ( var k in this.domreg_data.result.data.docs_specific ) {
          if ( docs_hash ) {
  ...
  constructor(
      ...
      var req_tld_list = this.http.get("/rest/v1/domain_registration/enom/tld_list");
      req_tld_list.pipe(
        timeout(180000))
        .subscribe( result => {
            this.dataSourceTld.data =result.json().result.product_list;
            this.is_loading.tld_list = false
            for ( var i = 0, item; item = result.json().result.product_list[i++]; ) {
